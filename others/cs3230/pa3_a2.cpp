...
const int MAX_SUM = MAX_N * MAX_VAL / 2;
...
void solve() {
    ...
    // Initialize the dp bitset
    dp.reset();
    dp[0] = 1;

    FOR(n) {
        // Shift the previous subsets to the left by v[i] bits and combine with v[i]
        dp |= (dp << v[i]);
    }

    // Find the maximum subset sum that is less than or equal to the total sum / 2
