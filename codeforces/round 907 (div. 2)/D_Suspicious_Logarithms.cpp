...
ll sum_of_powers(ll p, ll base) {
    if (p == 0) return 0;
    ll ans = 0, left = base, right = min(p, base * base - 1), power = 1;
    __int128_t pw = base * base;

    while (left <= p) {
        add_mod(ans, (right - left + 1) % MOD * power % MOD);
        pw *= base;
        power++;
        left = right + 1;
        right = min((__int128_t)p, pw - 1);
    }

    return ans;
}

ll sum_of_ranges(ll p) {
    if (p <= 3) return 0;
    ll ans = 0, base = 2;
    __int128_t pw = 4;

    for (ll left = 4, right = min(p, 7LL); left <= p; left = right + 1, right = min((__int128_t)p, left + pw - 1)) {
        add_mod(ans, sum_of_powers(right, base) - sum_of_powers(left - 1, base));
        pw <<= 1;
        base++;
    }
...
void solve() {
    ll left, right;
    read(left, right);
    print((sum_of_ranges(right) - sum_of_ranges(left - 1) + MOD) % MOD);
}
