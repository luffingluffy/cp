...
bool is_k_flower(int n, int m, const vector<vector<int>>& adj_list) {
    if (n != m) {
        return false;
    }
    ...
    for (const auto& v : adj_list) {
        if (v[0] < v[1]) {
            unique_edges.insert({v[0], v[1]});
        } else {
            unique_edges.insert({v[1], v[0]});
        }
    }
    return (int)unique_edges.size() == n;
}
...
void solve() {
    ...
    cout << (is_k_flower(n, m, adj_list) ? "YES" : "NO") << '\n';
}
