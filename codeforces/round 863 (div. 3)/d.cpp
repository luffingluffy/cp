...
bool solve(ll f1, ll f2, ll x, ll y, bool vert = true) {
    if (x == y && y == 1) {
        return true;
    }
    if (vert) {
        if (y <= f1 && y > (f2 - f1)) {
            ...
            y = f2 - y + 1 > f1 ? y : f2 - y + 1;
            return solve(f1, f2 - f1, x, y, !vert);
        }
    } else {
        if (x <= f2 && x > (f1 - f2)) {
            return false;
        } else {
            x = f1 - x + 1 > f2 ? x : f1 - x + 1;
            return solve(f1 - f2, f2, x, y, !vert);
        }
    }
}
...
int main() {
    ...
    while (t--) {
        ll n, x, y;
        cin >> n >> x >> y;
        cout << (solve(fibs[n], fibs[n + 1], x, y) ? "YES" : "NO") << endl;
    }
