...
ll perimeter(ll n, ll x, ll y) {
    return min(n - max(x, y) + 1, min(x, y));
}

ll dist(ll n, ll x1, ll y1, ll x2, ll y2) {
    return abs(perimeter(n, x1, y1) - perimeter(n, x2, y2));
}

void solve() {
    ll n, x1, y1, x2, y2;
    cin >> n >> x1 >> y1 >> x2 >> y2;
    cout << dist(n, x1, y1, x2, y2) << endl;
}
