...
long long to_base9(long long k) {
    long long res = 0;
    long long factor = 1;

    while (k > 0) {
        long long cur_digit = k % 9;
        if (cur_digit >= 4) {
            cur_digit++;  // Increment the digit if it is 4 or more to exclude the digit 4
        }
        res += cur_digit * factor;
        factor *= 10;
        k /= 9;
    }
