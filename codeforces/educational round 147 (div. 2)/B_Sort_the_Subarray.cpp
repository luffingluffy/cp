...
void solve() {
    ...
    int minLeft = 0;
    while (minLeft < size && a[minLeft] == b[minLeft]) {
        minLeft++;
    }

    int minRight = size - 1;
    while (minRight >= 0 && a[minRight] == b[minRight]) {
        minRight--;
    }

    while (minLeft > 0 && b[minLeft] >= b[minLeft - 1]) {
        minLeft--;
    }

    while (minRight < size - 1 && b[minRight] <= b[minRight + 1]) {
        minRight++;
    }

    cout << minLeft + 1 << " " << minRight + 1 << '\n';
}
