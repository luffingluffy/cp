...
void solve() {
    int n, t;
    read(n, t);
    vector<int> durations(n), entertainment_values(n);
    read(durations);
    read(entertainment_values);

    int max_entertainment = -1;
    int selected_video = -1;
    int time_spent = 0;

    for (int i = 0; i < n; ++i) {
        if (time_spent + durations[i] <= t) {
            if (entertainment_values[i] > max_entertainment) {
