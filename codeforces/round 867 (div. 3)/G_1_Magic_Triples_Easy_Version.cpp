...
void solve() {
    ...
    for (auto it = freq.rbegin(); it != freq.rend(); it++) {
        auto p = *it;
        for (ll j = 2; j <= 1000; j++) {
            if (j * p.first > 1e6) break;
            if (freq.find(p.first * j) != freq.end()) {
                dp[p.first * INT_MAX + j] += freq[p.first * j] * p.second;
                if (dp.find(p.first * j * INT_MAX + j) != dp.end()) {
                    ans += dp[p.first * j * INT_MAX + j] * p.second;
                }
            }
        }
    }
