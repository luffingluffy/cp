...
void solve() {
    ...
    vi count(26);
    vi totalCount(26);
    int sum = 0, maxCount = 0, maxTotalCount = 0;

    for (int i = 0; i < n / 2; i++) {
        if (s[i] == s[n - i - 1]) count[s[i] - 'a']++;
        totalCount[s[i] - 'a']++;
        totalCount[s[n - i - 1] - 'a']++;
    }
    ...
    for (int i : totalCount) maxTotalCount = max(maxTotalCount, i);

    if (maxTotalCount > n / 2 || n % 2 == 1) {
        print(-1);
    } else {
        print(maxCount > sum - maxCount ? maxCount : sum / 2 + (sum % 2));
    }
}
