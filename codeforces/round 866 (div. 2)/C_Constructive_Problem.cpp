...
int get_mex(vector<int>& arr) {
    ...
    // Find the first non-occurring integer (mex)
    int mex = 0;
    while (mex < size && freq[mex])
        mex++;
    return mex;
}
...
void solve() {
    ...
    if (find(a.begin(), a.end(), mex + 1) != a.end()) {
        // Find indices with value equal to mex + 1
        for (int i = 0; i < n; i++) {
            if (a[i] == mex + 1)
                indicesToRemove.push_back(i);
        }

        // Replace values in the range from first to last occurrence of mex + 1 with mex
        for (int i = indicesToRemove.front(); i <= indicesToRemove.back(); i++)
            a[i] = mex;

        // Check if the mex of the updated array is equal to mex + 1
        if (get_mex(a) == mex + 1)
            print("Yes");
        else
            print("No");
