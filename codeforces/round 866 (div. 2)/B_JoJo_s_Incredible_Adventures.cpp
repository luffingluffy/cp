...
void solve() {
    ...
    ll n = s.size();
    ll onesCount = count(s.begin(), s.end(), '1');

    // If the string contains only ones, return the area of the square formed by ones
    if (onesCount == n) {
        print(n * n);
        return;
    }

    ll maxConsecutiveOnes = 0, currentConsecutiveOnes = 0, iterations = 0;
    while (true) {
        if (s[iterations % n] == '1') {
        ...
        maxConsecutiveOnes = max(maxConsecutiveOnes, currentConsecutiveOnes);
        iterations++;
        if (iterations >= n && s[iterations % n] == '0') break;
    }

    maxConsecutiveOnes = max(maxConsecutiveOnes, currentConsecutiveOnes);
    ll maxRectangleArea = ((maxConsecutiveOnes + 1) * (maxConsecutiveOnes + 1)) / 4;
