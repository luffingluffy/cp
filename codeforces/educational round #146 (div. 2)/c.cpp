...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        int j;
        cin >> j;
        vals.push_back(make_pair(i + 1, j));
    }
    ...
    // Distribute values between two sets
    for (int i = 0; i < n; i++) {
        if ((a.size() + 1) * s1 * vals[i].second > (b.size() + 1) * s2 * vals[i].second) {
            b.push_back(vals[i].first);
            bCurr += (b.size() + 1) * s2 * vals[i].second;
        } else {
            a.push_back(vals[i].first);
            aCurr += (a.size() + 1) * s1 * vals[i].second;
        }
    }
