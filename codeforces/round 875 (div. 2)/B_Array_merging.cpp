...
void solve() {
    ...
    int prev_a = a[0], cnt_a = 1;
    for (int i = 1; i < n; i++) {
        if (prev_a == a[i]) {
            cnt_a++;
        } else {
            freq_a[prev_a] = max(freq_a[prev_a], cnt_a);
            prev_a = a[i];
            cnt_a = 1;
        }
    }
    ...
    int prev_b = b[0], cnt_b = 1;
    for (int i = 1; i < n; i++) {
        if (prev_b == b[i]) {
            cnt_b++;
        } else {
            freq_b[prev_b] = max(freq_b[prev_b], cnt_b);
            prev_b = b[i];
            cnt_b = 1;
        }
    }
    ...
    int ans = 0;
    for (int x : a) {
        ans = max(ans, freq_a[x] + freq_b[x]);
    }
    for (int x : b) {
        ans = max(ans, freq_b[x] + freq_a[x]);
    }
